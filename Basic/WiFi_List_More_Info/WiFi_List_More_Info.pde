 ...
 * This is a good opportunity to obtain the list of WiFi networks through getNetworkListSortedBySignal().
 *
...
void wifiScanFinished() {
  ...
  wifiNetworks = wifiList.getNetworkListSortedBySignal();
  // you could also just retrieve the unsorted list like this
  // wifiList.getNetworkList();
  for(ScanResult item : wifiNetworks) {
    printInfo(item);
  }
}
